...
class RequestHandler
{
    ...
    private array $allowed_actions = [
        'getProducts'       => ['ep' => 'products',                  'methods' => ['GET'],           'auth' => 'none'],
        'getProduct'        => ['ep' => 'products/(?P<id>\d+)',      'methods' => ['GET'],           'auth' => 'none'],
        'getOrders'         => ['ep' => 'orders',                    'methods' => ['GET'],           'auth' => 'customer'],
        'getCustomer'       => ['ep' => 'customers/(?P<id>\d+)',     'methods' => ['GET'],           'auth' => 'customer_self'],
        'updateOrder'       => ['ep' => 'orders/(?P<id>\d+)',        'methods' => ['PUT', 'PATCH'],  'auth' => 'customer_owner'],
        'createOrder'       => ['ep' => 'orders',                    'methods' => ['POST'],          'auth' => 'optional'],
        'customerLogin'     => ['ep' => null,                        'methods' => ['POST'],          'auth' => 'none'],
        'customerRegister'  => ['ep' => null,                        'methods' => ['POST'],          'auth' => 'none'],
    ];
    ...
    {
        register_rest_route('woosecureproxy/v3', '/proxy', [
            'methods'             => 'POST',
    ...
    public function handle_request(WP_REST_Request $request): WP_REST_Response
    {
        ...
        // Header validation
        if ($error = $this->validate_headers($headers, $request_id)) {
            return $error;
        }

        // HMAC signature verification
        if ($error = $this->verify_signature($headers, $raw_body, $request_id)) {
            return $error;
        }

        // Replay attack protection
        if ($error = $this->check_replay($headers['nonce'], $request_id)) {
            return $error;
        }
        ...
        $customerId = $this->getCustomerIdFromJwt($request);

        // Special built-in auth endpoints
        if (in_array($action, ['customerLogin', 'customerRegister'])) {
            return $this->handleCustomerAuth($action, $data, $request_id);
        }

        // Action whitelist & method check
        $actionConfig = $this->allowed_actions[$action] ?? null;
        if (!$actionConfig || !in_array($method, $actionConfig['methods'], true)) {
            return $this->error('action_not_allowed', 'Requested action or method is not permitted', 403, $request_id);
        }

        $authMode = $actionConfig['auth'];

        // Enforce login when required
        if (in_array($authMode, ['customer', 'customer_self', 'customer_owner']) && !$customerId) {
            return $this->error('unauthenticated', 'Login required for this action', 401, $request_id);
        }

        // Self-access enforcement
        if ($authMode === 'customer_self') {
            $requestedId = $data['id'] ?? 0;
            if ((int)$requestedId !== $customerId) {
                return $this->error('forbidden', 'You can only access your own customer record', 403, $request_id);
            }
        }

        // Order ownership enforcement
        if ($authMode === 'customer_owner') {
            $orderId = $data['id'] ?? 0;
            $order   = wc_get_order($orderId);
            if (!$order || $order->get_customer_id() !== $customerId) {
                return $this->error('forbidden', 'You can only modify your own orders', 403, $request_id);
            }
        }

        // Auto-fill customer context
        if ($action === 'createOrder' && $customerId) {
            $data['customer_id'] = $customerId;
        }
        if ($action === 'getOrders' && $customerId) {
            $data['customer'] = $customerId;
        }

        // Rate limiting
        if ($error = $this->rate_limit($headers['app_token'], $ip, $action, $request_id)) {
            return $error;
        }
        ...
        $wc_url = rest_url('wc/v3/' . $actionConfig['ep']);
        ...
        $args = [
            'method'      => $method,
            'timeout'     => 30,
            'headers'     => array_merge(KeyManager::get_auth_header(), [
        ...
        if ($code >= 500 && (!defined('WP_DEBUG') || !WP_DEBUG)) {
            $resp_body = wp_json_encode(['error' => 'Internal server error']);
        }
        ...
        $resp = new WP_REST_Response([
            'success' => $code < 400,
            'data'    => json_decode($resp_body, true) ?: $resp_body,
        ], $code);
    ...
    private function extract_headers(WP_REST_Request $request): array
    {
        ...
        array_walk($raw, static function (&$v) {
            $v = trim($v);
    ...
    {
        $required = ['app_token', 'timestamp', 'nonce', 'signature'];
    ...
    private function rate_limit(string $app_token, string $ip, string $action, string $req_id): ?WP_REST_Response
    {
        ...
        foreach (['ip' => $ip, 'app' => $app_token] as $type => $id) {
     ...
     */
    private function getCustomerIdFromJwt(WP_REST_Request $request): ?int
    {
     ...
     */
    private function handleCustomerAuth(string $action, array $data, string $request_id): WP_REST_Response
    {
        if ($action === 'customerLogin') {
            ...
            return new WP_REST_Response(['success' => true, 'jwt' => $jwt, 'customer_id' => $wc_customer->get_id()], 200);
        }
        ...
        if ($action === 'customerRegister') {
            ...
            return new WP_REST_Response(['success' => true, 'jwt' => $jwt, 'customer_id' => $user_id], 201);
        }
    ...
    private function get_allowed_tokens(): array
    {
        ...
        $tokens = is_array($tokens) ? array_values(array_unique(array_filter($tokens))) : ['mobile-v2'];
    ...
    private function get_rate_limits(): array
    {
        ...
        if ($json !== '' && ($custom = json_decode($json, true)) && is_array($custom)) {
