...
namespace WooSecureProxy\Helpers {
	...
	// Namespaced error_log() override: captures every line the Logger emits
	// into $GLOBALS['wsp_test_log_buffer'] so tests can assert on log output.
	if ( ! function_exists( __NAMESPACE__ . '\\error_log' ) ) {
		/**
		 * Captures error_log calls under WooSecureProxy\Helpers.
		 *
		 * @param string $message Log line.
		 */
		function error_log( string $message ): bool {
			$GLOBALS['wsp_test_log_buffer'][] = $message;
			return true;
		}
	}
}
